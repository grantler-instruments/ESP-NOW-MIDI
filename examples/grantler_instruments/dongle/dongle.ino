#include "./config.h"
#include "enomik_dongle.h"

#if HAS_DISPLAY == 1
#include "SSD1306Display.h"
#include "Menu.h"
#endif

#include <AceButton.h>
using namespace ace_button;

enomik::Dongle _dongle;

#if HAS_DISPLAY == 1
SSD1306Display display;
GrantlerMenu _menu(_dongle);
#endif

AceButton _btnCursor;
AceButton _btnSelect;

void handleEvent(AceButton*, uint8_t, uint8_t);

void setup() {
  Serial.begin(115200);
  delay(1000);

#if HAS_DISPLAY == 1
  display.setDongle(&_dongle);
  display.setMenu(&_menu);
  _dongle.setDisplay(&display);
#endif

  pinMode(BTN_CURSOR, INPUT_PULLUP);
  pinMode(BTN_SELECT, INPUT_PULLUP);
  _btnCursor.init(BTN_CURSOR, HIGH, 0);
  _btnSelect.init(BTN_SELECT, HIGH, 1);

  ButtonConfig* buttonConfig = ButtonConfig::getSystemButtonConfig();
  buttonConfig->setEventHandler(handleEvent);
  buttonConfig->setFeature(ButtonConfig::kFeatureClick);
  buttonConfig->setFeature(ButtonConfig::kFeatureLongPress);
  buttonConfig->setFeature(ButtonConfig::kFeatureRepeatPress);
  buttonConfig->setFeature(ButtonConfig::kFeatureSuppressAfterLongPress);
  buttonConfig->setLongPressDelay(BTN_LONG_PRESS_MS);

  // Optional: hard-add a known peer (or wait for auto-discovery)
  // uint8_t clientMac[6] = { 0x84, 0xF7, 0x03, 0xF2, 0x54, 0x62 };
  // _dongle.addPeer(clientMac);

  // --- Bridge filters (optional) -----------------------------------------
  // midi_message fields: status, channel (1-16), firstByte, secondByte
  // Return true to forward (after any edits), false to drop.
  // Keep filters non-blocking. Does not apply to send*() inject APIs.
  //
  // Drop channel 10 (GM drums) before it reaches the computer:
  // _dongle.setToHostFilter([](midi_message& msg) {
  //   return msg.channel != 10;
  // });
  //
  // Force everything from the computer onto MIDI channel 1:
  // _dongle.setFromHostFilter([](midi_message& msg) {
  //   msg.channel = 1;
  //   return true;
  // });
  //
  // Drop sustain pedal (CC 64) from the computer; pass everything else:
  // _dongle.setFromHostFilter([](midi_message& msg) {
  //   if (msg.status == MIDI_CONTROL_CHANGE && msg.firstByte == 64) {
  //     return false;
  //   }
  //   return true;
  // });
  //
  // Clear a filter later:
  // _dongle.setToHostFilter(nullptr);
  // _dongle.setFromHostFilter(nullptr);

  _dongle.begin();
}

void loop() {
  _btnCursor.check();
  _btnSelect.check();
#if HAS_DISPLAY == 1
  if (_menu.tick()) {
    _dongle.invalidateDisplay();
  }
#endif
  _dongle.loop();
}

void handleEvent(AceButton* button, uint8_t eventType, uint8_t /*buttonState*/) {
#if HAS_DISPLAY == 1
  static bool openedMenuThisPress = false;

  if (eventType == AceButton::kEventPressed ||
      eventType == AceButton::kEventClicked ||
      eventType == AceButton::kEventLongPressed ||
      eventType == AceButton::kEventRepeatPressed) {
    _menu.noteActivity();
  }

  if (_menu.page() == GrantlerMenu::Page::Status) {
    if (eventType == AceButton::kEventPressed) {
      _menu.onEnter();
      openedMenuThisPress = true;
      _dongle.invalidateDisplay();
    }
    return;
  }

  if (openedMenuThisPress) {
    if (eventType == AceButton::kEventClicked ||
        eventType == AceButton::kEventLongPressed) {
      openedMenuThisPress = false;
    }
    return;
  }

  const uint8_t id = button->getId();
  if (id == 0) {
    if (eventType == AceButton::kEventPressed ||
        eventType == AceButton::kEventRepeatPressed) {
      _menu.onCursor();
      _dongle.invalidateDisplay();
    }
    return;
  }

  if (eventType == AceButton::kEventClicked) {
    _menu.onEnter();
    _dongle.invalidateDisplay();
  } else if (eventType == AceButton::kEventLongPressed) {
    _menu.onLongPress();
    _dongle.invalidateDisplay();
  }
#else
  (void)button;
  (void)eventType;
#endif
}
